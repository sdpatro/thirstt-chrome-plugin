...
xhr_sendClip.onreadystatechange = function () {
  ...
  if (xhr_sendClip.readyState == 4 && xhr_sendClip.status == 200){
    var resJson = JSON.parse(xhr_sendClip.responseText);
    if (resJson.status == 1){
      status = "success";
    }
    else{
...
var subSimple = chrome.contextMenus.create({"title": "Open in simple mode", "parentId": main, "onclick": function (e) {
  openSimple(e.pageUrl);
